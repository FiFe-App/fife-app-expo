...
- Env: copy `example.env` to `.env` and set `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY_ANDROID`, `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY_WEB` (restricted per platform — Android by package+SHA-1, web by HTTP referrer; iOS uses Apple Maps natively, no Google key needed), `EXPO_PUBLIC_LOCATIONIQ_API_KEY` (address search, from locationiq.com).
